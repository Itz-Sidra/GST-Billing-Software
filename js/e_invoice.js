...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document.getElementById(
    "invoice-number"
  ).value = `INV-${new Date().getFullYear()}${random}`;
});
...
function calculateRowTotal(input) {
  ...
  const taxRate =
    parseFloat(row.querySelector('select[name="tax[]"]').value) || 0;
...
function calculateTotals() {
  ...
  for (let i = 0; i < rows.length; i++) {
    const qty =
      parseFloat(rows[i].querySelector('input[name="qty[]"]').value) || 0;
    const price =
      parseFloat(rows[i].querySelector('input[name="price[]"]').value) || 0;
    const taxRate =
      parseFloat(rows[i].querySelector('select[name="tax[]"]').value) || 0;
...
function generateInvoice() {
  ...
  document.getElementById("preview-invoice-number").innerText =
    document.getElementById("invoice-number").value;
  document.getElementById("preview-invoice-date").innerText = formatDate(
    document.getElementById("invoice-date").value
  );
  document.getElementById("preview-due-date").innerText =
    document.getElementById("due-date").value
      ? formatDate(document.getElementById("due-date").value)
      : "N/A";
  document.getElementById("preview-ack-date").innerText = formatDate(
    document.getElementById("invoice-date").value,
    true
  );

  // Seller details
  document.getElementById("preview-seller-name").innerText =
    document.getElementById("seller-name").value;
  document.getElementById("preview-seller-address").innerText =
    document.getElementById("seller-address").value;
  document.getElementById("preview-seller-gstin").innerText =
    document.getElementById("seller-gstin").value;

  const sellerStateSelect = document.getElementById("seller-state");
  document.getElementById("preview-seller-state-code").innerText =
    sellerStateSelect.value;
  document.getElementById("preview-seller-state-name").innerText =
    sellerStateSelect.options[sellerStateSelect.selectedIndex].text;

  // Buyer details
  document.getElementById("preview-buyer-name").innerText =
    document.getElementById("buyer-name").value;
  document.getElementById("preview-buyer-address").innerText =
    document.getElementById("buyer-address").value;
  document.getElementById("preview-buyer-gstin").innerText =
    document.getElementById("buyer-gstin").value;

  const buyerStateSelect = document.getElementById("buyer-state");
  document.getElementById("preview-buyer-state-code").innerText =
    buyerStateSelect.value;
  document.getElementById("preview-buyer-state-name").innerText =
    buyerStateSelect.options[buyerStateSelect.selectedIndex].text;
  ...
  // Update totals
  document.getElementById("preview-subtotal").innerText = `₹${subtotal.toFixed(
    2
  )}`;
  ...
  document.getElementById("preview-grand-total").innerText = `₹${total.toFixed(
    2
  )}`;
  ...
  ).innerText = `${amountInWords(total)} Only`;
...
function validateForm() {
  ...
  for (let i = 0; i < itemRows.length; i++) {
    ...
    if (!desc || !hsn || !qty || !price) {
      isValid = false;
      if (!desc)
        itemRows[i].querySelector(
          'input[name="item-desc[]"]'
        ).style.borderColor = "red";
      if (!hsn)
        itemRows[i].querySelector('input[name="hsn[]"]').style.borderColor =
          "red";
      if (!qty)
        itemRows[i].querySelector('input[name="qty[]"]').style.borderColor =
          "red";
      if (!price)
        itemRows[i].querySelector('input[name="price[]"]').style.borderColor =
          "red";
    }
  }
...
function downloadInvoice() {
  ...
  alert(
    "This would download the invoice as a PDF file in a real implementation. For demo purposes, we will use the Print dialog which can be saved as PDF."
  );
  printInvoice();
}
